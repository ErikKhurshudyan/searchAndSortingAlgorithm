...
static void BM_VectorLinearSearch(benchmark::State& state) {
    ...
    while (state.KeepRunning()) {
        std::vector<int>::iterator found = binarySearch(v.begin(), v.end(), key);
...
static void BM_ListLinearSearch(benchmark::State& state) {
    ...
    int key = SIZE - 1;
    while (state.KeepRunning()) {
        std::list<int>::iterator found = binarySearch(l.begin(), l.end(), key);
        benchmark::DoNotOptimize(found);
    }
}
...
static void BM_DequeLinearSearch(benchmark::State& state) {
    ...
    int key = SIZE - 1;
    while (state.KeepRunning()) {
        std::deque<int>::iterator found = binarySearch(d.begin(), d.end(), key);
        benchmark::DoNotOptimize(found);
    }
}
...
static void BM_SetLinearSearch(benchmark::State& state) {
    ...
    int key = SIZE - 1;
    while (state.KeepRunning()) {
        std::set<int>::iterator found = binarySearch(s.begin(), s.end(), key);
        benchmark::DoNotOptimize(found);
    }
}
